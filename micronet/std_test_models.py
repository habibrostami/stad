...
from settings import  TRAIN_FOLDER, TEST_FOLDER, VALIDATION_FOLDER,AUGMENTED_TRAIN_FOLDER, AUGMENTED_VAL_FOLDER
from settings import DIRECTORY_PATH_DISTRIBUT_CHECK, DIRECTORY_PATH

from settings import checkpoint_filepath,last_checkpoint_filepath
from settings import checkpoint_directory
...
validation_set = validation_datagen.flow_from_directory(DIRECTORY_PATH+ VALIDATION_FOLDER,
                                            target_size=(IMAGE_HEIGHT, IMAGE_WIDTH),
                                            batch_size=BATCH_SIZE,
                                            class_mode='categorical', shuffle=False)
...
#val_class_weights [0] = 1
#val_class_weights [1] = 1
...
def get_refresh_classifier():
    ...
    classifier = models.get_pretrained_Xception(state_shape, action_shape, learning_rate=LEARNING_RATE,
                                                qlearning=False, no_freez_layers=0)
    classifier.summary()
    return classifier
...
save_model (classifier,last_checkpoint_filepath)

def run_super_epoch(model,  super_epoch_no):
    checkpoint_filepath = checkpoint_directory + 'super_epoch_'+str(super_epoch_no)+ 'std_best_model.h5'
